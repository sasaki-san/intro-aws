...
それぞれの意味を忘れてしまった場合は，ハンズオン1に戻って復習していただきたい．
...
次のようなコマンドで Docker を起動する．

[source, bash]
----
docker run -it -v ./:~/.ssh registry.gitlab.com/tomomano/intro-aws:latest
----

こうすることで，クライアントマシン本体のワーキングディレクトリ (`./`) と， Docker の `~/.ssh` というディレクトリがシンクロした形になり，相互的にファイルをやり取りすることができる．
====
